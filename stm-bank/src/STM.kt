...
class TxVar<T>(initial: T) {
    ...
    fun openIn(
        ...
        while (true) {
            ...
            val curValue =
                curLoc.valueIn(tx) { owner ->
                    owner.abort() // on conflicts, retake tx ownership
                }

            if (curValue == TxStatus.ACTIVE) continue // retry again after conflict resolution
            val newValue = update(curValue as T)
            val newLoc = Loc(curValue, newValue, tx)
            if (loc.compareAndSet(curLoc, newLoc)) {
                if (tx.status == TxStatus.ABORTED) throw AbortException // someone aborted us, validating zombie tx
                return newValue
            }
        }
    }
